...
public class HttpClientExecutor implements HttpExecutor {
    ...
    public HttpResponseTemplate execute(HttpRequestTemplate request) throws IOException {
        ...
        // 当前上下文中的servletResponse
        HttpServletResponse servletResponse =
                Optional.ofNullable((ServletRequestAttributes) RequestContextHolder.getRequestAttributes())
                .map(ServletRequestAttributes::getResponse).orElse(null);
        if (servletResponse != null && !servletResponse.containsHeader(X_Request_ID)) {
            Header[] headers = httpResponse.getHeaders(X_Request_ID);
            if(headers != null && headers.length > 0){
                servletResponse.setHeader(X_Request_ID, headers[0].getValue());
            }
        }
        ...
        return new HttpResponseTemplate(status, null, reason, stream, (int) length);
    }
